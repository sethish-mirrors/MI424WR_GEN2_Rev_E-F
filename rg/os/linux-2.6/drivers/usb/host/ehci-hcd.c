...
/* ADI's VOX160 USB host controller do not interface with PCI bus */
...
static int ehci_reset (struct ehci_hcd *ehci)
{
	...
	{
	    static void solos_reset(struct ehci_hcd *ehci);
	    solos_reset(ehci);
	}
#endif
	if (retval)
	...
	if (ehci_is_FSL(ehci))
	{
	    extern void mpc8349_usb_reset(void);
	    mpc8349_usb_reset ();
	}
#endif	
	...
	if (hcd->self.controller->bus == &pci_bus_type) {
		...
		while (temp && count--) {
			...
			case 1:			/* BIOS/SMM/... handoff */
				printk("if (bios_handoff (ehci, temp, cap) !="
				"0)\n");
//					return -EOPNOTSUPP;
				break;
