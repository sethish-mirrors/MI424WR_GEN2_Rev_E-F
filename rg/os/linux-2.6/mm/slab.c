...
int kmem_cache_has_free_space(kmem_cache_t *cachep)
{
	...
	for_each_online_node(i) {
		...
		if (l3) {
			spin_lock_irq(&l3->list_lock);
			ret += !list_empty(&l3->slabs_partial) ||
				!list_empty(&l3->slabs_free);
			spin_unlock_irq(&l3->list_lock);
		}
	}
...
unsigned int kmem_get_free_ram_size(void)
{
	...
	si_meminfo(&si);
	cached = get_page_cache_size() - total_swapcache_pages - si.bufferram;
	if (cached < 0)
		cached = 0;
	return (si.freeram + cached) << PAGE_SHIFT;
}
