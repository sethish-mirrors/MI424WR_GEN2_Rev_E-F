...
static void display_status(int count, int col)
{
	...
	printf("\n\e[7m  PID USER     STATUS    VMSIZE     RSS  PPID %%CPU %%MEM COMMAND\e[0m\n");
#else
	printf("\n\e[7m  PID USER     STATUS    VMSIZE     RSS  PPID %%MEM COMMAND\e[0m\n");
#endif
...
#ifdef FEATURE_CPU_USAGE_PERCENTAGE
		printf("%5d %-8s %s %12lu %s %5d %2d.%d %2u.%u ",
			s->pid, s->user, s->state, s->vsize, rss_str_buf, s->ppid,
			s->pcpu/10, s->pcpu%10, pmem/10, pmem%10);
#else
		printf("%5d %-8s %s %12lu %s %5d %2u.%u ",
			s->pid, s->user, s->state, s->vsize, rss_str_buf, s->ppid,
			pmem/10, pmem%10);
#endif
