...
void hardware_features(void)
{
    ...
    if (IS_HW("MC524WR"))
    {
	int rev = token_get("CONFIG_MC524WR_REV");
	...
	case 0x2A:
	    if (token_get("CONFIG_HW_ETH_LAN"))
		dev_add("eth0", DEV_IF_CN3XXX_ETH, DEV_IF_NET_INT);
	    else if (token_get("CONFIG_HW_SWITCH_LAN"))
	    {
		token_set_m("CONFIG_HW_SWITCH_BCM53XX");
	    ...
	    if (token_get("CONFIG_HW_ETH_WAN"))
		dev_add_switch_port("eth0", "eth5", 4, DEV_IF_NET_EXT);
		...
		dev_can_be_missing("clink0");
		token_set_y("CONFIG_ENTROPIC_EN2210_MII");
		dev_add("eth1", DEV_IF_MOCA_MII, DEV_IF_NET_EXT);
		token_set_y("CONFIG_RG_LIMITED_CPP");
		token_set_y("CONFIG_RG_CXX");
	    }
	    break;
	case 0x5A:
		...
		dev_add("eth0", DEV_IF_KS8995M_HW_SWITCH, DEV_IF_NET_INT);
	    }

	    if (token_get("CONFIG_HW_ETH_WAN"))
		dev_add("eth1", DEV_IF_CN3XXX_ETH, DEV_IF_NET_EXT);
		...
		dev_add("clink0", DEV_IF_MOCA_PCI, DEV_IF_NET_INT);
		dev_can_be_missing("clink0");
		dev_add("clink1", DEV_IF_MOCA_PCI, DEV_IF_NET_EXT);
	    ...
	    break;
	default:
	    fprintf(stderr, "Unknown MC524WR revision\n");
	    exit(1);
	}
